...
def check_and_set_default_args(command_args):
    ...
    if not(hasattr(command_args, 'valid_shards')) or command_args.valid_shards is None:
        command_args.valid_shards = 5
    ...
    assert not command_args.valid_shards % command_args.num_threads, (
        'Please make the command_args.num_threads commensurate with command_args.valid_shards')
    assert command_args.train_directory is not None
    assert command_args.valid_directory is not None
...
def main(command_args):
    """
    command_args需要有以下属性：
    command_args.train_directory    ----训练集所在的文件夹,下面的每个文件夹的名字代表label名称,里面是训练集图片
    command_args.valid_directory    ----验证集所在的文件夹,下面的每个文件夹的名字代表label名称,里面是验证集图片
    command_args.labels_file        ----输出label.txt文件,每一行代表一个label名称
    command_args.output_directory   ----输出tfrecord文件位置

    command_args.train_shards       ----训练集划分的份数
    command_args.valid_shards       ----验证集划分的份数
    command_args.num_threads        ----线程数,必须是上面两个参数的约数
    command_args.class_label_base   ----很重要！真正的tfrecord中,每个class的label号从多少开始,默认为0
    command_args.dataset_name       ----字符串,tfrecord输出的前缀
    ...
    # Run it!
    _process_dataset('valid', command_args.valid_directory, command_args.valid_shards,
                     command_args.labels_file, command_args)
    _process_dataset('train', command_args.train_directory, command_args.train_shards,
                     command_args.labels_file, command_args)
